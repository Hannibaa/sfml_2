...
*            +----+----+----+----+----+----+----+----+----+
			...
			: Generation is 98% for this i keep the code
...
int main() {
	...
	auto remove_adjacent_wall = [&](int x, int y, int oldx, int oldy) {
		...
		case -1: return;
		case 1: // other = 5
			grid[NZ(x, y)].set_wall(1);
			grid[NZ(oldx, oldy)].set_wall(5);
			break;
		case 5: 
			grid[NZ(x, y)].set_wall(5);
			grid[NZ(oldx, oldy)].set_wall(1);
			break;
		case 7: 
			grid[NZ(x, y)].set_wall(7);
			grid[NZ(oldx, oldy)].set_wall(3);
			break;
		case 3: 
			grid[NZ(x, y)].set_wall(3);
			grid[NZ(oldx, oldy)].set_wall(7);
			break;
		}
		};
	...
	while (window.isOpen()) {
		...
		//calculate visited neibors
		visitedNeibor = visited_neibor(nx, ny);

		if (Bool["Back"]()) {
			if (!prev.empty()) {
				// check at least one neibor is not visited
				if (visitedNeibor == 4) {
					prev.pop();
					if (!prev.empty()) {
						nx = prev.top().x;
